...
{
    eraseElement(dotInOrder,mm->abandoned);
    if (scanSN%2 == 0){
        eraseElement(dotPositionEven,mm->abandoned);
        correspondPointEven.clear();
        for (size_t e = 0; e < mm->refinedCorr.size(); e++){
            //if (!isBelongTo(e,mm->abandoned))
            correspondPointEven.push_back(mm->refinedCorr[e]);
        }
    }
    else{
        eraseElement(dotPositionOdd,mm->abandoned);
        correspondPointOdd.clear();
        for (size_t r = 0; r < mm->refinedCorr.size(); r++){
            correspondPointOdd.push_back(mm->refinedCorr[r]);
        }
    }
    delete mm;
...
vector<Point2f> DotMatch::subPixel(Mat img, vector<vector<float>> vec)
{
    ...
    for (size_t i = 0; i < vec.size(); i++){
        ...
        if (p.x < MIDUP || p.y < MIDUP || p.x > img.cols-MIDUP || p.y > img.rows-MIDUP)
            continue;
        ...
        */
        ...
        if (img.at<uchar>((int)p.y, (int)p.x) != 0){
            ...
            if (yd >= MIDUP || yu >= MIDUP || xr >= MIDUP || xl >= MIDUP){
                ...
                bool mismatch = false;//如果新坐标与之前已有坐标距离过近，判定为误匹配，删除该点
                for (size_t u=0;u<out.size();u++){
                    if (fabs(p.x-out[u].x)<EDGEUP && fabs(p.y-out[u].y)<EDGEUP)
                        mismatch = true;
                }
                if (mismatch)
                    continue;
